function [IT] = ABM_main(size,nm,np,na,nsteps,fmode,outImages)
...
%ecolab(size,nm,np,nsteps)
%size = size of model environmnet in km (sugested value for plotting
%purposes =50)
%nm - initial number of monkey agents
%np - initial number of parasite agents
%na - initial number of monkey agents with immunity
...
%create_environment.m)
    ...
    global N_IT IT_STATS ENV_DATA MESSAGES

    % If number of input variables less than 6
    if nargin == 4 
        fmode=true;
        outImages=false;
    elseif nargin == 5
...
%     random_selection(1);                %randomises random number sequence (NOT agent order). If input=0, then simulation should be identical to previous for same initial values
    [agent]=create_agents(nm,np,na);       %create nm rabbit and np fox agents and places them in a cell array called 'agents'
    create_messages(agent);       %sets up the initial message lists
    initialise_results(nm,np,na,nsteps);   %initilaises structure for storing results
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     %MODEL EXECUTION
    for n_it=1:nsteps                   %the main execution loop
        N_IT=n_it;
        [agent,n]=agnt_solve(nm,agent);     %the function which calls the rules
%         plot_results(nsteps,agent);
        if IT_STATS.tot_p(N_IT) == 0 || IT_STATS.tot_m(N_IT) == 0
            break;
        end
    end
...
%         end
IT = IT_STATS;
eval(['save results_nm_' num2str(nm) '_np_' num2str(np) '.mat IT_STATS ENV_DATA' ]);
clear global
end
