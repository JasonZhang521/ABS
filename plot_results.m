...
function plot_results(nsteps,agent)
...
disp(strcat('No. monkeys dying = ',num2str(IT_STATS.died_m(N_IT+1))))
disp(strcat('No. parasites decreases = ',num2str(IT_STATS.died_p(N_IT+1))))
disp(strcat('No. monkey anti = ',num2str(IT_STATS.anti_m(N_IT+1))))
...
n=nm(N_IT+1)+np(N_IT+1);             %current agent number
f2=figure(2);
set(f2,'Units','Normalized');
set(f2,'Position',[0.5 0.5 0.45 0.4]);

subplot(2,1,1),cla
subplot(2,1,1),plot((1:N_IT+1),nm(1:N_IT+1),col{1});
subplot(2,1,1),axis([0 nsteps 0 1.1*max(nm)]);
subplot(2,1,2),cla
subplot(2,1,2),plot((1:N_IT+1),np(1:N_IT+1),col{2});
subplot(2,1,2),axis([0 nsteps 0 1.1*max(np)]);
subplot(2,1,1),title('No. live monkeys');
subplot(2,1,2),title('No. live parasites');
% drawnow

% 2D plot of environment and agents
f3=figure(3);
set(f3,'Units','Normalized');
set(f3,'Position',[0.05 0.05 0.4 0.4]);

n_length = size(agent,2);
for i = 1:n_length
    if (isa(agent{i},'monkey'))
        plot(agent{i}.pos(1),agent{i}.pos(2),'r.');
        axis([0 50 0 50]);
        hold on;
    elseif (isa(agent{i},'parasite')) 
        plot(agent{i}.pos(1),agent{i}.pos(2),'bo');
        axis([0 50 0 50]);
        hold on;        
    end  
   title('environment and agents');
end
hold off;
