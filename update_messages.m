...
for cn=1:temp_n
    ...
    if dead==0                          %if agent is not dead
        ...
        MESSAGES.pos(cn,:)=pos;
        if isa(agent{cn},'monkey')
            MESSAGES.atype(cn)=1;
            IT_STATS.tot_m(N_IT+1)=IT_STATS.tot_m(N_IT+1)+1;
            if agent{cn}.state == -1 % with immunity
                IT_STATS.anti_m(N_IT+1)=IT_STATS.anti_m(N_IT+1)+1;
            end
        elseif isa(agent{cn},'parasite')
            MESSAGES.atype(cn)=2;
...
IT_STATS.died_m(N_IT+1) = IT_STATS.tot_m(N_IT) - IT_STATS.tot_m(N_IT+1);
IT_STATS.died_p(N_IT+1) = IT_STATS.tot_p(N_IT) - IT_STATS.tot_p(N_IT+1);